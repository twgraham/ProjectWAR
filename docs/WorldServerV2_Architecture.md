...
20. [Cross-Cutting Design Principles](#20-cross-cutting-design-principles)
21. [Shared Infrastructure Projects](#21-shared-infrastructure-projects)
...
├── Network/
│   ├── ClientState.cs                  — enum: NotConnected → Playing → Disconnected
...
| **Component** | An optional behavior or state container that can be attached to an entity at runtime. Implements `IComponent`. Components that need per-tick updates also implement `ITickable`. Required state (like health) is a direct field, not a component. |
...
sequenceDiagram
    ...
    Note over C,R: Client renders world, region streams nearby entities via visibility
```
...
## 20. Cross-Cutting Design Principles
...
## 21. Shared Infrastructure Projects
