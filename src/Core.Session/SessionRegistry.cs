...
public sealed class SessionRegistry
{
    ...
    /// </exception>
    public GameSession CreateSession(IConnectionContext connection)
    {
        lock (_writeLock)
        {
            if (_freeIds.Count == 0)
    ...
    /// <param name="accountId">The authenticated account.</param>
    /// <returns>
    /// The previously registered session for <paramref name="accountId"/>, or <c>null</c>
    /// if no conflict existed.
    /// </returns>
    public GameSession? SetSessionAccount(GameSession session, uint accountId)
    {
        ...
        lock (_writeLock)
        {
            ...
            if (_byAccountId.TryGetValue(accountId, out var existing) && existing != session)
                displaced = existing;

            session.AccountId = accountId;
            _byAccountId[accountId] = session;
    ...
    public void Remove(GameSession session)
    {
        ...
        lock (_writeLock)
        {
            ...
            if (session.AccountId is not null)
            {
