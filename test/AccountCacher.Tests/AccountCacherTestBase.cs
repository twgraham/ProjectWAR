...
using AccountMgrClient = AccountMgr.AccountMgrClient;
using CreateAccountRequest = global::CreateAccountRequest;
using CreateAccountResponse = global::CreateAccountResponse;
using GetAccountRequest = global::GetAccountRequest;
using GetAccountResponse = global::GetAccountResponse;

namespace AccountCacher.Tests;

public abstract class AccountCacherTestBase : IAsyncLifetime
{
    private MySqlContainer? _mysqlContainer;
    private IHost? _host;
    protected GrpcChannel? Channel;
    protected AccountMgrClient? Client;
    ...
    public async ValueTask InitializeAsync()
    {
        ...
        _host = Host.CreateDefaultBuilder()
            .ConfigureWebHostDefaults(builder =>
            {
                builder.ConfigureKestrel(opts =>
                        opts.ListenLocalhost(0, o => { o.UseHttps(); })) // Use dynamic port
        ...
        var address = addresses?.Addresses.FirstOrDefault() ?? "https://localhost:5001";
        ...
        Client = new AccountMgrClient(Channel);
    ...
    protected async Task<int> InsertTestAccountAsync(string username, string password, string email = "test@test.com", int gmLevel = 0, int banned = 0)
    {
        using var connection = new MySqlConnection(ConnectionString);
        await connection.OpenAsync();
    ...
    protected async Task<int> InsertTestRealmAsync(byte realmId, string name, string address = "127.0.0.1", int port = 10300)
    {
        using var connection = new MySqlConnection(ConnectionString);
        await connection.OpenAsync();
    ...
    protected async Task<int> InsertIpBanAsync(string ip, int expire)
    {
        using var connection = new MySqlConnection(ConnectionString);
        await connection.OpenAsync();
